...
public interface RouteRepository extends JpaRepository<Route, Integer> {
	...
	@Query("select b from Route b where (?1 is null or ?1=' ' or b.origin like ?1) and (?2 is null or ?2=' 'or b.destination like ?2) and (?3 is null or b.vehicle.type = ?3) and (?4 is null or b.maxLuggage = ?4) and (?5 is null or b.driver.pets = ?5) and (?6 is null or b.driver.childs = ?6) and (?7 is null or b.driver.smoke = ?7) and (?8 is null or b.driver.music = ?8)")
	Collection<Route> searchRoutes(String origin, String destination, VehicleType vehicleType, LuggageSize maxLuggage, Boolean pets, Boolean childs, Boolean smoke, Boolean music);
